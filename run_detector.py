...
class NpySearchEngine:
    ...
    def search(self, query, top_k=5):
        ...
        for idx in top_indices:
            ...
            original_text = str(
                doc.get("original") or doc.get("text", "")
            )
...
def analyze_text(text, search_engine, detector, top_k=5):
    ...
    for i, sentence in enumerate(sentences, start=1):
        ...
        results.append({
            ...
            "sentence_plagiarism_percentage": round(
                detection["final_score"] * 100,
                2
            ),
    ...
    plagiarised = sum(
        1 for r in results
        if r["prediction"] == "plagiarism"
    )

    avg_score = (
        sum(r["sentence_plagiarism_percentage"] for r in results)
        / len(results)
        if results else 0.0
    )
...
_search_engine = NpySearchEngine(
    _documents,
    EMB_PATH
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run plagiarism detection on plain text input."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of nearest documents to inspect"
    )
