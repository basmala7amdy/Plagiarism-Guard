...
def build_response(
    ...
    """
    Convert detector output into frontend-compatible response.
    """
    ...
    for sentence, result in zip(
        ...
        if (
            ...
            highlighted_parts.append(
                f'<span class="plagiarized">{escaped}</span>'
            )
        else:
        ...
        "similarity_timeline": [
            {
                "segment": idx + 1,
                "similarity": r.get(
                    "sentence_plagiarism_percentage",
                    0.0,
                ),
            }
...
async def root():
    return {
        "message": "Plagiarism Guard API running"
    }
